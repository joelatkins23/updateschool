    ...
    * ********************************************************** */
    public function index() {
        ...
        if ($_POST) {
            ...
            $this->data['students'] = $this->Aptitudenote_Model->get_student_list($school_id,$school->academic_year_id,$student_id,$exam_id,$course_id,$class_id,$section_id,$period_id);
            ...
            $this->data['alldata'] = $this->Aptitudenote_Model->get_exam_history($school_id, $school->academic_year_id, $exam_id,$course_id,$class_id,$section_id,$period_id);
    ...
    * ********************************************************** */
        ...
        if ($_POST) {            
            ...
            if (!empty($_POST['students'])) {
                ...
                foreach ($_POST['students'] as $key => $value) {            
                    $student_id=$value;
                    $studentinfo = $this->Aptitudenote_Model->get_student_list($school_id,$school->academic_year_id, $student_id, $exam_id,$course_id,$class_id,$section_id,$period_id);
                    ...
                    if($_POST['avg_grade_point'][$value]>=9.5 && $_POST['avg_grade_point'][$value]<=10){
                        $item['avg_grade_point']=10;
                    }else{
                        $item['avg_grade_point']=$_POST['avg_grade_point'][$value];
                    }                    
                    $item['grade_id']=$_POST['grade_id'][$value];
                    $item['result_status']=($item['avg_grade_point']>=10)? 'approved' :'No Approved';
                ...
                $this->data['students'] = $this->Aptitudenote_Model->get_student_list($school_id, $school->academic_year_id, "",$exam_id,$course_id,$class_id,$section_id,$period_id);       
            }
            ...
            $this->data['academic_year']= $this->db->get_where('academic_years', array('id'=>$school->academic_year_id))->row()->session_year;
            $this->data['classes'] = $this->result->get_list('classes', array('school_id'=>$school_id), '','', '', 'id', 'ASC');
            $this->data['courses']=$this->result->get_list('courses', array('school_id'=>$school_id), '','', '', 'id', 'ASC');
            $this->layout->title($this->lang->line('aptitude') . ' ' . $this->lang->line('note') . ' | ' . SMS);
            $this->layout->view('aptitude_note/index', $this->data);
    }
    public function report() {
        ...
        foreach ($array['students'] as $key => $value) {            
            ...
            $item['exam_id']= $exam_id;
            $item['status']= ($array['avg_grade_point'][$value]>=9.5)? 'Approved' : 'No Approved';
