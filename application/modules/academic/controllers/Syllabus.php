...
class Syllabus extends MY_Controller {
    ...
    public function index($class_id = null) {
        ...
        // $this->data['courses']=$this->syllabus->get_list('courses');
        $condition = array();
        $condition['status'] = 1; 
        $this->data['class_list'] = $this->syllabus->get_list('classes', $condition, '','', '', 'id', 'ASC');
        $this->data['courses'] = $this->syllabus->get_list('courses', $condition, '','', '', 'id', 'ASC');       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    $class = $this->syllabus->get_single('classes', array('id' => $data['class_id'], 'school_id'=>1));
    ...
    private function _prepare_syllabus_validation() {
        ...
        $this->form_validation->set_rules('course_id', $this->lang->line('course'), 'trim|required');   
