    ...
    * ********************************************************** */
    public function listsubscriptions() {
        ...
        if ($_POST) {           
            ...
            $this->data['academic_year_id'] = $academic_year_id;
            $this->data['course_name'] = $this->db->get_where('courses', array('id'=>$course_id))->row()->name;
            ...
            $this->data['subscriptionslist'] = $this->report->get_student_course_report($school_id, $academic_year_id,$course_id,$period, $class_id,$section_id);
        ...
        $this->data['courses']=$this->report->get_list('courses');
        $condition['status'] = 1;   
        $condition['school_id']=1; 
        $this->data['classes'] = $this->report->get_list('classes', $condition, '','', '', 'id', 'ASC');
        $this->data['academic_years']=$this->report->get_list('academic_years', array('school_id' =>  $school_id), '','', '', 'id', 'ASC');
        $this->data['report_url'] = site_url('report/listsubscriptions');
        $this->layout->title($this->lang->line('list') . ' ' . $this->lang->line('subscriptions') . ' ' .$this->lang->line('report') . ' | ' . SMS);
        $this->layout->view('subscriptions/index', $this->data);
        
    }
    

    public function listadmissao() {
        ...
        $this->data['classes'] = $this->report->get_list('classes', $condition, '','', '', 'id', 'ASC');
        $this->data['academic_years']=$this->report->get_list('academic_years', array('school_id' =>  $school_id), '','', '', 'id', 'ASC');
        $this->data['report_url'] = site_url('report/listadmissao');
        $this->layout->title($this->lang->line('list') . ' ' . $this->lang->line('subscriptions') . ' ' .$this->lang->line('report') . ' | ' . SMS);
        $this->layout->view('admissao/index', $this->data);
        
    }


    public function listadmit() {
        ...
        if ($_POST) {
            ...
            $this->data['admitlist'] = $this->report->get_admit_student_course_report($school_id, $academic_year_id,$course_id,$period);
        ...
        $this->data['courses']=$this->report->get_list('courses');
        $this->data['academic_years']=$this->report->get_list('academic_years', array('school_id' =>  $school_id), '','', '', 'id', 'ASC');
        $this->data['report_url'] = site_url('report/listadmit');
        $this->layout->title($this->lang->line('list') . ' ' . $this->lang->line('subscriptions') . ' ' .$this->lang->line('report') . ' | ' . SMS);
        $this->layout->view('admit/index', $this->data);
    ...
    * ********************************************************** */
    public function sinvoice() {
        ...
        $this->layout->view('student/sinvoice', $this->data);
          ...
          /*****************Function studentdailypayment**********************************
    * @type            : Function
    * @function name   : sbalance
    * @description     : Load stduent daily payment 
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function studentdailypayment() {
        ...
        if ($_POST) {
            ...
            $this->data['view']=TRUE;
            $this->data['school'] = $this->report->get_school_by_id($school_id);            
            $this->data['allpaymentlist'] = $this->report->get_student_daily_report($start_date,$end_date);
        ...
        $this->data['report_url'] = site_url('report/sudentpayment');
        $this->layout->title($this->lang->line('student') . ' ' . $this->lang->line('daily') . ' ' .$this->lang->line('payment') . ' | ' . SMS);
        $this->layout->view('student/dailypayment', $this->data);
        
    }   
    /*****************Function studentnotpayment**********************************
    * @type            : Function
    * @function name   : sbalance
    * @description     : Load stduent not payment 
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function studentnotpayment() {
        ...
        $this->data['view']=TRUE;
        $this->data['school'] = $this->report->get_school_by_id($school_id);            
        $this->data['allnotpaymentlist'] = $this->report->get_student_not_report($start_date,$end_date);
        $this->data['start_date']=$start_date;
        $this->data['end_date']=$end_date;
        ...
        $this->data['report_url'] = site_url('report/studentnotpayment');
        $this->layout->title($this->lang->line('student') . ' ' . $this->lang->line('not') . ' ' .$this->lang->line('payment') . ' | ' . SMS);
        $this->layout->view('student/notpayment', $this->data);
        
    }   
    /*****************Function studentagepayment**********************************
    * @type            : Function
    * @function name   : studentagepayment
    * @description     : Load age, class,grade                
    *                    with various filtering options
    *                    and process to load balance report   
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function studentagepayment() {
        ...
        if ($_POST) {
            ...
            $this->data['view']=TRUE;
            $this->data['school'] = $this->report->get_school_by_id($school_id);  
            $academic=$this->report->get_list('academic_years', array('id'=>$year));
            $this->data['allpaymentagelist'] = $this->report->allpaymentagelist($academic[0]->start_year,$class_id,$age);
            $this->data['year']=$year;
            $this->data['year_date']=$academic[0]->start_year;
            $this->data['class_id']=$class_id;
            $this->data['classname']=$this->report->get_list('classes', array('id'=>$class_id));
            $this->data['age']=$age;
        }
        ...
        //     $this->data['classes'] = $this->report->get_list('classes', $condition, '', '', '', 'id', 'ASC');
        // } 
        $this->data['academic_year'] = $this->report->get_list('academic_years', array('school_id'=>'1'));
        $this->data['classes'] = $this->report->get_list('classes', array('school_id'=>'1'),'', '', '', 'id', 'ASC');
        $this->data['report_url'] = site_url('report/studentagepayment');
        $this->layout->title($this->lang->line('student') . ' ' . $this->lang->line('age') . ' ' .$this->lang->line('payment') . ' | ' . SMS);
        $this->layout->view('student/agepayment', $this->data);
        
    } 
    function  get_age_by_year_class(){
        $year=$this->input->post("year");
        $class_id=$this->input->post("class_id");
        $age=$this->input->post("age");
        $ages=$this->report->get_age_by_year_class($year,$class_id,$age);
        $str = '<option value="">--' . $this->lang->line('select') . '--</option>';
        $select = 'selected="selected"';
        if (!empty($ages)) {
            foreach ($ages as $obj) {   
                
                $selected = $age == $obj->name ? $select : '';
