...
class Student_Model extends MY_Model {
    ...
    public function get_provinciallist() {
        $this->db->select('S.*');
        $this->db->from('tb_provincias  AS S');
        $this->db->order_by('S.id_p', 'ASC');       
        return $this->db->get()->result();
    }
    public function check_section($course_id = null, $class_id = null, $period = null){
        $query="select * from sections w 
            left join (select count(b.section_id)num, b.section_id from students a
            left join enrollments b on (a.id=b.student_id) 
            where course_id='".$course_id."' and class_id='".$class_id."' and period='".$period."' group by section_id order by section_id) q on (w.id=q.section_id) order by id desc";
    ...
    public function get_student_list($class_id = null, $school_id = null, $academic_year_id = null){            
        ...
        $this->db->select('S.*, SC.school_name, E.roll_no, E.class_id, U.username, U.role_id,  C.name AS class_name, SE.name AS section, DE.name AS course_name');
    ...
    public function get_student_list_canceled($class_id = null, $school_id = null, $academic_year_id = null){
