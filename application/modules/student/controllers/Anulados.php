...
class Student extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_student_validation() {
        ...
        $this->form_validation->set_rules('course_id', $this->lang->line('course_id'), 'trim|required');        $this->form_validation->set_rules('type_id', $this->lang->line('school'), 'trim|required');        
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        if (!empty($student)) {
            ...
            // delete student data
            $this->student->update('students',array('status' => 0), array('id' => $id));

            // delete student login data
            $this->student->update('users', array('status' => 0),array('id' => $student->user_id));

            // delete student enrollments
            $this->student->update('enrollments',array('status' => 0),  array('student_id' => $student->id));

            // delete student hostel_members
            $this->student->update('hostel_members',array('status' => 0), array('user_id' => $student->user_id));

            // delete student transport_members
            $this->student->update('transport_members',array('status' => 0), array('user_id' => $student->user_id));

            // delete student library_members
            $this->student->update('library_members',array('status' => 0), array('user_id' => $student->user_id));
    ...
    * ********************************************************** */
    private function __insert_enrollment($insert_id) {
        ...
        $checkdata=$this->student->check_section($this->input->post('course_id'),$this->input->post('class_id'),$this->input->post('period'));
        $section_id=0;
        foreach ($checkdata as $key=>$arr){
            switch ($arr){
                case ($arr->num<=45):
                    $section_id =$arr->id;
                    break;                
