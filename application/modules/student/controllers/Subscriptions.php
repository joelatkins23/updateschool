...
class Subscriptions extends MY_Controller {
    ...
    * ********************************************************** */        
    public function move_admint_student() {
        ...
        $student_data=$this->student->get_list('students_subscription', array('id' =>$student_id), '', '', '', 'id', 'ASC');
        ...
        if($student_insertid){
            ...
            $checkdata=$this->Subscriptions_Model->check_section($student_data[0]->course_id, $enrollmentsinfo[0]->class_id,$student_data[0]->period);
            $section_id=0;
            ...
            if($insert_enrollments){
                ...
                $this->student->update('student_payment', array('student_id' => $student_insertid,'student_type' => 'admitted'), array('student_id' => $student_id,'student_type' => 'subscription'));
            }
    ...
    * ********************************************************** */
    private function __insert_enrollment($insert_id) {
        ...
        $checkdata=$this->Subscriptions_Model->check_section($this->input->post('course_id'),$this->input->post('class_id'),$this->input->post('period'));
        $section_id=0;
        foreach ($checkdata as $key=>$arr){
            switch ($arr){
                case ($arr->num<=45):
