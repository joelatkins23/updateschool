...
select r.created_at,q.* from student_payment_details q
inner join student_payment r on (q.studentpayment_id=r.id)) w where  created_at BETWEEN date_format('".$start_date."','%Y-%m-%d') and date_format('".$end_date."','%Y-%m-%d') and type='month'  group by studentpayment_id) b
        on (a.id=b.studentpayment_id)
        left join (select studentpayment_id ,sum(item_price)product_totoal from(
select r.created_at,q.* from student_payment_details q
inner join student_payment r on (q.studentpayment_id=r.id)) w where  created_at BETWEEN date_format('".$start_date."','%Y-%m-%d') and date_format('".$end_date."','%Y-%m-%d') and type='product' and item_name<>'Inscrição' and item_name<>'Matriculas' group by studentpayment_id) c
        on (a.id =c.studentpayment_id)
        left join (select *,sum(item_price)registration  from(
select r.created_at,q.* from student_payment_details q
inner join student_payment r on (q.studentpayment_id=r.id)) w where  created_at BETWEEN  date_format('".$start_date."','%Y-%m-%d') and date_format('".$end_date."','%Y-%m-%d')  and type='product' and item_name='Inscrição'  group by studentpayment_id) br
        on (a.id =br.studentpayment_id)
        left join (select *,sum(item_price)admit  from(
select r.created_at,q.* from student_payment_details q
inner join student_payment r on (q.studentpayment_id=r.id)) w where  created_at BETWEEN date_format('".$start_date."','%Y-%m-%d') and date_format('".$end_date."','%Y-%m-%d') and type='product' and item_name='Matriculas'  group by studentpayment_id) ba
...
inner join student_payment r on (q.studentpayment_id=r.id)) w where  created_at BETWEEN date_format('".$start_date."','%Y-%m-%d') and date_format('".$end_date."','%Y-%m-%d') and type='product' and item_name='Matriculas'  group by studentpayment_id) ba
    ...
    public function get_student_not_report($start_date,$end_date, $classe, $curso){
        ...
        left join courses d on (a.course_id=d.id where curse_id = '$curso')
    ...
    public function allpaymentagelist($year =null,$class_id = null, $age = null){
        ...
        where student_type='admitted' and date_format(created_at, '%Y')='".$year."') w ";
        if($class_id){
        ...
        where student_type='subscription' and date_format(created_at, '%Y')='".$year."') w ";
        if($class_id){
            $sql.=" where w.class_id='".$class_id."'";
        }
        if($age){
    ...
    public function get_admit_student_course_report($school_id = null, $academic_year_id = null, $course_id = null, $period = null,$class_id = null,$section_id = null){       
        $sql = "SELECT a.name AS student_name, c.name as course_name,a.period,d.name as class_name, e.name as section_name from students a 
        left join enrollments as b on (a.id=b.student_id) 
        left join courses as c on (a.course_id=c.id)
        left join classes d on (b.class_id=d.id)
       
        left join sections e on (b.section_id=e.id) where a.school_id='".$school_id."' and a.status='1' and b.academic_year_id='".$academic_year_id."'";
       if($course_id){
