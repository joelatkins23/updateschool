...
if (!function_exists('get_exam_semester_result')) {

    function get_exam_semester_result($school_id, $exim_id, $student_id, $academic_year_id, $class_id, $semester_id, $section_id = null) {
        $ci = & get_instance();
        $ci->db->select('ER.*, G.name');
        $ci->db->from('exam_results AS ER');  
        $ci->db->join('grades AS G', 'G.id = ER.grade_id', 'left');  
        $ci->db->where('ER.school_id', $school_id);
        $ci->db->where('ER.exam_id', $exim_id);
        $ci->db->where('ER.semester_id', $semester_id);
        $ci->db->where('ER.class_id', $class_id);
