...
class Ajax extends My_Controller {
    ...
    public function get_guardian_by_school() {
        ...
        $selected = $guardian_id == '10000' ? $select : '';
        $str.='<option value="10000" ' . $selected . '>Myself</option>';
        echo $str;
    }
    ...
    public function get_course_by_school() {
        
        $school_id  = $this->input->post('school_id');
        $course_id  = $this->input->post('course_id');
        
       $courses = $this->ajax->get_list('courses', array('status'=>1, 'school_id'=>$school_id), '','', '', 'id', 'ASC'); 
